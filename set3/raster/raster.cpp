...
void raster(vertex verts[3], void (*drawPixel)(int, int, float *)) {
  ...
  for (int y = (int)((yMin > 0) ? yMin : 0); y < yMax && y < yRes; y++) {
    for (int x = (int)((xMin > 0) ? xMin : 0); x < xMax && x < xRes; x++) {
      ...
      // could insert code here to do clipping
      // would need to...maybe init raster with clipping BB
      // then either convert these back to world coords
      // wait, but also need Z...
